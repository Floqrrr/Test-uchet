...
const EditConfigurator = ({ history }) => {
  ...
  const Ondel = (e)=> {
    e.preventDefault();
    deleteTodo(history.location.pathname.toString().slice(-1))
    history.push("/")
  }

  const OnSubmit = (e) => {
    e.preventDefault();
    updateTodo(formData, history.location.pathname.toString().slice(-1))
    history.push("/")
        ...
        <Row>
          ...
          <Col className="col-md-6">
            <Form onSubmit={OnSubmit}>
              ...
              <Button className="ml-2" onClick={Ondel} variant="outline-danger">
                Удалить
              </Button>
            </Form>
          </Col>
